...
sumByMultiMovieYn=spark.sql(f"SELECT any_value(load_dt) load_dt ,multiMovieYn, {col_set[:-1]} FROM movie_info GROUP BY multiMovieYn")
sumByMultiMovieYn.write.mode("overwrite").partitionBy("load_dt").parquet("/home/root2/data/movie/sum-multi/")
sumByMultiMovieYn.show()

sumByRepNationCd=spark.sql(f"SELECT any_value(load_dt) load_dt ,repNationCd, {col_set[:-1]} FROM movie_info GROUP BY repNationCd")
